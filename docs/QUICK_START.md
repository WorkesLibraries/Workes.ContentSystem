...
The default workflow uses a bounded FIFO structure where the structure assigns IDs. Keyed workflows use caller-provided typed IDs. Later structures may be threaded, indexed, grouped, persistent, grid-like, or forum-like.
...
`ContentManagerBase` is the shared read and lookup ancestor for manager-agnostic code. Most users should construct `ContentManager` or `KeyedContentManager<TId>` directly.
